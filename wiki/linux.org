...
* Tmp
** git checkout from multiple remotes
From [[https://stackoverflow.com/questions/1783405/how-do-i-check-out-a-remote-git-branch][this stack-overflow question]]:
...
In ubuntu, you install =ibus-pinyin= and run =ibus-setup=, run =ibus restart=,
logout and login. In setting, adding "Chinese" as input source is not
enough. You need to add "Chinese - Pinyin", see [[https://www.pinyinjoe.com/linux/ubuntu-18-gnome-chinese-setup.htm][this post]].

This would not work in Emacs. You need to start emacs, according to [[https://mollermara.com/blog/ibus-emacs/][this post]]:
...
** RedmiBook wifi Atheros QCA6174 driver

Actually [[https://askubuntu.com/questions/607707/ath10k-installation][this post]] solved my problem. You need:
1. https://github.com/kvalo/ath10k-firmware, put it into
   /lib/firmware/ath10k/QCA6174 and rename necessary files accordingly. hw2.1 is
   not important.
2. The comment 2 in [[https://bugzilla.redhat.com/show_bug.cgi?id=1294263#c2][this post]] attached a windows binary. THIS IS IMPORTANT. Put
   it to *both* =hw3.0/board.bin= and =hw3.0/board-2.bin=

I was about to help [[https://bbs.deepin.org/forum.php?mod=viewthread&tid=184723&page=2][this post]] but I have to wait 5 hours to post anything
because of newly created account. This is stupid, this probably won't come back
to my mind.
...
=~/.local/share/applications/test.desktop= with following:
...
** setup frp reverse proxy
Use https://natfrp.com, create tunnels, note the IDs and your token. Download
their specialized frpc client, and start the service on the machine behind NAS:
...
For example, I would love to expose several ports, thus I'm creating one for
each port:
...
|-----+-------+-----------------------+------------------------------+-------+--------------+-------------------+--------------------|
| #7  |       | 福州电信              | (cn-fz-dx.sakurafrp.com)     |       |              |               300 |                    |
| #8  | Y     | 俄勒冈AWS             | (us-or-aws.sakurafrp.com)    |       |              |               300 |                    |
| #13 | Y     | 洛杉矶CN2             | (us-la-kr.sakurafrp.com)     |       |              |               280 |                    |
| #23 | Y     | 日本东京IIJ-2         | (jp-tyo-dvm-2.sakurafrp.com) |       |              |    150 (unstable) |                    |
| #24 | Y     | 镇江多线2             | (cn-zj-bgp-3.sakurafrp.com)  |       |              |                20 |                    |
| #28 |       | 枣庄多线3             | (cn-zz-bgp-3.sakurafrp.com)  |       |              |                35 |                    |
|-----+-------+-----------------------+------------------------------+-------+--------------+-------------------+--------------------|
| #3  | Y     | [VIP]香港DMIT         | (cn-hk-dmit.sakurafrp.com)   |       |              |                75 |                    |
| #14 | Y     | [VIP]上海多线         | (cn-sh-bgp.sakurafrp.com)    | frpsh |   5901:39574 |                27 |                    |
| #25 |       | [VIP]杭州多线         | (cn-hz-bgp.sakurafrp.com)    | frphz |     22:43632 |                25 |                    |
|-----+-------+-----------------------+------------------------------+-------+--------------+-------------------+--------------------|
| #17 | Y     | [VIP][高级]深圳多线   | (cn-sz-bgp.sakurafrp.com)    |       |              |                37 |                    |
| #19 | Y     | [VIP][高级]香港阿里云 | (cn-hk-ali.sakurafrp.com)    |       |              |                65 |                    |


*** previous writing reference: expose local machine without public IP
...
I'm not sure if the speed or stability of these international providers. For
chinese providers:
...
- https://www.natfrp.com/: a free frc service provider
  - This looks very good. You basically choose a server on their console, and
    run local client and choose the same console.


However, I'm not sure the performance of all these models. All traffic has to go
through public server? The public server should really just act as a lookup service.

But even though I got assigned a public IP, all the ports do not seem to be
open. Thus, even if the DDNS services got the IP correct and up-to-date, that is
not supposed to work still.

UPDATE: actually china mobile did not assign me a public IP. The IP seen on my
router is 100.99.240.168, while my public IP in whatismyipaddress and is ip138
is 36.161.131.36. Thus, I don't even has a public IP, not to say a statis public IP.
...
**** SSH tunneling
That keeps alive a SSH connection between your local machine and a public
server, and forward all traffic to public server to your local machine.
...
#+begin_example
ssh -nN -R 8888:localhost:8889 remoteuser@public-server.com
#+end_example

Here, the 8888 port on public server will be forwarded to localhost 8889.

However, there are problems:
1. The SSH connection must be kept alive. If it broke for any reason, we have to
   setup a logic to restart it.
2. This only setup one port at a time. We'll need to run it multiple times for
   more ports.
3. There's no redirection based on domain name.
...
Looks like this is very promising, it can even work in P2P mode for large data
transfer, and it has a dashboard.
...
*** enable service
From [[https://wiki.archlinux.org/index.php/TigerVNC][this reference on archwiki]], add =/etc/systemd/system/vncserver@.service=:
...
** The docker manager, Portainer
This is actually great:
https://www.portainer.io/documentation/quick-start/, run via:
