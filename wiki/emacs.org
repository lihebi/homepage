...
*** syntax highlight for racket in editor
*** auto-completion
*** doc accessibility
...
Then you will find the racket kernel in
#+begin_example
jupyter notebook
#+end_example

Problems:
- [ ] I actually can install the kernel, and run a console (associated with a
  file) to an open file.
- [ ] But the #lang is not supported, and there's a PR for that. But the console
  seems to be associating with one file.
- [ ] Also, the writing to tmp folder (and seems all writings to any folders are
  denied) seems to be not smooth. I also need to figure out how to do
  visualization.

And for BHDL project, the freerouting actually opens GUI window. How should I
appraoch that?

And I want to open KiCAD for results, then I'll need to access the remote
disk. How should I set that up? I can of course download it from jupyterlab or
retrieve it via SSH. But I would love a seemless experience, like showing a link
in jupyter notebook.

I probably want to use only native notebook functionality.

Actually I can still code in Emacs, and evaluate some visualization staff in
notebook. That way, I can just program in Emacs via SSH. But that would be
sensitive to network connectivity, and I have to deal with Tmux session
management.

**** ZeroMQ
- https://zeromq.org/: ZeroMQ An open-source universal messaging library. This
  is pretty interesting.
- and the racket binding is pretty amazing piece of code:
  https://github.com/jeapostrophe/zeromq It uses at-exp lang?? Anyway.
...
language APIs and runs on most operating systems. ZeroMQ is from iMatix and is
LGPLv3 open source.
#+end_quote
