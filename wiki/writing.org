...
I can use =hebi-gen-bib= to generate a =bibliography:xxx= for used
citations. Then, in generated =.tex= file, execute =tex-bibtex-file= to generate
bib cache. Then, regenerate pdf will have citations.

To export the used citations to a separate bib file, remove the
=bibliography:xxx= line and run =org-ref-extract-bibtex-to-file=.
...
#+BEGIN_EXAMPLE
#+ATTR_LATEX: :options [t]{0.33\textwidth}
#+BEGIN_minipage
...
#+END_minipage
#+END_EXAMPLE
...
Reference: [[https://orgmode.org/worg/org-tutorials/images-and-xhtml-export.html][a good tutorial]].
