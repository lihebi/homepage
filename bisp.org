...
However, abusing parenthesis creates readability problem, less elegent than
...
- hard to do *inline type annotations*. Prior art: typed racket, contracts,
  clojure specs
...
program elegent and extensible. Howver, in lisp dialects the support for
multi-dispatch is not that good due to no proper type systems, for example
generic method in racket and gerbil, defmulti in clojure.
...
In a word, Bisp is a lisp with a little less parenthesis, infix/postfix-notation
when approprite, and multi-dispatching.
